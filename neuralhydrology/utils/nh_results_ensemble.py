...
def _get_results_file(run_dir: Path, period: str = 'test', epoch: int = None) -> Path:
    ...
    if epoch is not None:
        dir_results_files = list(Path(run_dir).glob(
            f'{period}/model_epoch{str(epoch).zfill(3)}/{period}_results.p'))
    else:
        dir_results_files = list(Path(run_dir).glob(
            f'{period}/model_epoch*/{period}_results.p'))
...
def _main():
    ...
    parser.add_argument('--run-dirs', type=str, nargs='+',
                        help='Directories of the runs to be averaged.', required=True)
    parser.add_argument('--period', type=str,
                        choices=['train', 'validation', 'test'], default='test')
