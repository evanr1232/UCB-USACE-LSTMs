...
class UCB_trainer:
    ...
    def cross_validate(self, intervalMonth='October', intervalLength=2, validationLength=1, gap=False) -> dict:
        """
        ...
        arguments:
            ...
            intervalLength: optional, int, the length of the initial fold in uears, default is 2
        ...
        while i <= max_fold:
            ...
            self._config.update_config({'train_end_date': pd.to_datetime(f"{str(original_start_year + (intervalLength * i))}-{interval}-01", format="%Y-%b-%d")})
            self._config.update_config({'validation_start_date': pd.to_datetime(f"{str(original_start_year + (intervalLength * i) + gap)}-{interval}-02", format="%Y-%b-%d")})
            self._config.update_config({'validation_end_date': pd.to_datetime(f"{str(original_start_year + (intervalLength * i + validationLength) + gap)}-{interval}-01", format="%Y-%b-%d")})
